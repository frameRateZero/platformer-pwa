...
export default function FlatPlatformer() {
  ...
  const getLevelData = (levelNum: number): LevelData => {
    ...
    if (levelNum >= 10) {
      const levelVariations: { [key: number]: LevelData } = {
        10: {
          platforms: [
            { x: 0, y: 450, w: 220, h: 20 },
  ...
  const keys = useRef<{ left: boolean; right: boolean; up: boolean; jumpPressed: boolean }>({ left: false, right: false, up: false, jumpPressed: false });
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (godModeRef.current && e.ctrlKey && e.key >= '0' && e.key <= '9') {
        e.preventDefault();
      ...
      if (godModeRef.current && e.ctrlKey) {
        if (e.key === 'ArrowLeft') {
          e.preventDefault();
          const prevLevel = Math.max(1, levelRef.current - 1);
          setLevel(prevLevel);
  ...
  }, []); // Empty dependency array - event listeners never need to be re-added

  const drawPlayer = (ctx: CanvasRenderingContext2D, player: Player, camera: { x: number }, frame: number) => {
  ...
  const drawGoalFlag = (ctx: CanvasRenderingContext2D, flag: Platform, camera: { x: number }, frame: number) => {
  ...
  const drawEnemy = (ctx: CanvasRenderingContext2D, enemy: Enemy, camera: { x: number }, frame: number) => {
  ...
  const drawSpikes = (ctx: CanvasRenderingContext2D, spike: Platform, camera: { x: number }) => {
  ...
  const getSpikeHitbox = (spike: Platform): Platform => {
    return {
